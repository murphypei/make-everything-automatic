...
# # Media I/O:
# sudo apt-get install -y zlib1g-dev libjpeg-dev libwebp-dev libpng-dev libtiff5-dev jasper \
...
cd OpenCV && rm -rf build && mkdir build
cd build
...
if [ $OPENCV_CONTRIB = 'YES' ]; then
    cmake -DCMAKE_C_COMPILER=/usr/bin/clang -DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DCMAKE_CXX_FLAGS="-std=c++14 -stdlib=libc++" \
